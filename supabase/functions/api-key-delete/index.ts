...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === 'POST' || (req.method === 'DELETE' && req.headers.get('content-type')?.includes('json'))) {
      try {
    ...
    // Verify key exists and get details for audit
    ...
    if (deleteError) {
      ...
      return createErrorResponse(ErrorCode.DATABASE_ERROR, 'Failed to delete API key', 500);
    }
