...
SUPABASE_URL_NEW=https://xxxxxxxxxxxxx.supabase.co
SUPABASE_ANON_KEY_NEW=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
...
```bash
# Full database dump
pg_dump "postgresql://postgres:PASSWORD@db.hjplkyeuycajchayuylw.supabase.co:5432/postgres" \
  --schema=public \
  --data-only \
...
psql "postgresql://postgres:PASSWORD@db.xxxxxxxxxxxxx.supabase.co:5432/postgres" << EOF
...
export const getSupabaseClient = () => {
  ...
  const configs = {
    production: {
      url: process.env.SUPABASE_URL_OLD!,
      anonKey: process.env.SUPABASE_ANON_KEY_OLD!,
    },
    v2: {
      url: process.env.SUPABASE_URL_NEW!,
      anonKey: process.env.SUPABASE_ANON_KEY_NEW!,
    },
  };
...
export const getSupabaseClientWithRollout = (userId?: string) => {
  ...
  if (rolloutPercentage === 0) {
    ...
    return createClient(
      ...
      process.env.SUPABASE_ANON_KEY_OLD!
    );
  }
  ...
  if (rolloutPercentage === 100) {
    ...
    return createClient(
      ...
      process.env.SUPABASE_ANON_KEY_NEW!
    );
  }
  ...
  if (userId) {
    ...
    return useV2
      ? createClient(process.env.SUPABASE_URL_NEW!, process.env.SUPABASE_ANON_KEY_NEW!)
      : createClient(process.env.SUPABASE_URL_OLD!, process.env.SUPABASE_ANON_KEY_OLD!);
  }
