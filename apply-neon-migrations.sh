...
check_env_vars() {
    ...
    if [ -z "$MAAS_DATABASE_URL" ]; then
        print_error "MAAS_DATABASE_URL is not set"
        missing=1
    fi
    
    if [ -z "$SECURITY_DATABASE_URL" ]; then
        print_error "SECURITY_DATABASE_URL is not set"
        missing=1
    fi
    ...
    if [ $missing -eq 1 ]; then
        ...
        echo ""
        echo "export MAAS_DATABASE_URL='postgresql://neondb_owner:PASSWORD@ep-xxx.us-east-1.aws.neon.tech/app_lanonasis_maas?sslmode=require'"
        echo "export SECURITY_DATABASE_URL='postgresql://neondb_owner:PASSWORD@ep-xxx.us-east-1.aws.neon.tech/security_service?sslmode=require'"
        exit 1
    fi
}
...
main() {
    ...
    test_connection "$MAAS_DATABASE_URL" "MaaS Database" || exit 1
    test_connection "$SECURITY_DATABASE_URL" "Security Service Database" || exit 1
    echo ""
    ...
    if psql "$MAAS_DATABASE_URL" -c "CREATE EXTENSION IF NOT EXISTS vector;" > /dev/null 2>&1; then
    ...
    # Apply migration 002
    apply_migration "$MAAS_DATABASE_URL" "supabase/migrations/002_memory_service_maas.sql" "MaaS Database" || exit 1
    echo ""
    
    # Verify MaaS schema
    verify_migration "$MAAS_DATABASE_URL" "maas" "MaaS Database"
    echo ""
    ...
    if psql "$SECURITY_DATABASE_URL" -c "CREATE EXTENSION IF NOT EXISTS pgcrypto;" > /dev/null 2>&1; then
    ...
    # Apply migration 001
    apply_migration "$SECURITY_DATABASE_URL" "supabase/migrations/001_vendor_management.sql" "Security Service Database" || exit 1
    echo ""
    
    # Apply migration 003 (external vendor keys)
    apply_migration "$SECURITY_DATABASE_URL" "supabase/migrations/003_external_vendor_keys.sql" "Security Service Database" || exit 1
    echo ""
    ...
    psql "$MAAS_DATABASE_URL" -c "SELECT routine_name FROM information_schema.routines WHERE routine_schema = 'maas' ORDER BY routine_name;"
    echo ""
    
    print_status "Security Service Functions:"
    psql "$SECURITY_DATABASE_URL" -c "SELECT routine_name FROM information_schema.routines WHERE routine_name LIKE '%vendor%' ORDER BY routine_name;"
    echo ""
