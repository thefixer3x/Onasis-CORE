...
exports.handler = async (event, context) => {
  try {
    ...
    const corsHeaders = {
      ...
      'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Vendor, X-API-Key, x-project-scope',
    ...
    // Route based on path
    const response = await routeRequest(path, httpMethod, headers, requestBody, queryStringParameters);
...
async function routeRequest(path, method, headers, body, query) {
  ...
  if (path.includes('/chat/completions') && method === 'POST') {
    return {
      ...
      body: {
        error: {
          message: 'Chat completions endpoint not implemented in serverless mode',
