...
if (!supabaseUrl || !supabaseServiceKey) {
  console.error(
    "SUPABASE_DATABASE_URL/SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY/SUPABASE_SERVICE_KEY are required",
  );
}
...
const ENCRYPTION_KEY =
  process.env.KEY_ENCRYPTION_SECRET || "default-key-change-in-production";
        ...
        case "GET":
          if (id) {
            ...
            const { data: keys, error } = await supabase
              .from("vendor_api_keys")
              .select(
                "id, vendor_name, key_name, created_at, updated_at, last_used_at, is_active, description",
              )
              .eq("is_active", true)
              .order("vendor_name");
