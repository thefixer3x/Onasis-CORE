...
app.use((req, res, next) => {
  ...
  if (
    ...
    const hasValidKeys =
      ...
      Object.keys(req.body).some((key) => isNaN(parseInt(key)));
    ...
    if (Array.isArray(rawBody)) {
      console.log(
        "[maas-api] Body is array, reconstructing from character codes"
      );
    ...
    // Case 2: Body is object with only numeric keys (array-like object)
      ...
      console.log(
        "[maas-api] Body has numeric keys, reconstructing from character codes"
      );
    ...
    if (needsParsing && rawBody) {
      try {
        ...
        console.error(
          "[maas-api] Raw body (first 200 chars):",
...
const resolveOrganizationId = async (
  ...
  try {
    ...
    if (!orgError && orgData?.id) {
      console.log(
        "[maas-api] Found organization_id matching vendor_org_id:",
        orgData.id
      );
      return orgData.id;
    }
    ...
    if (!vendorError && vendorOrgData?.organization_id) {
      console.log(
        "[maas-api] Found organization_id from vendor_organizations:",
        vendorOrgData.organization_id
      );
    ...
    console.log(
      "[maas-api] Using vendor_org_id as organization_id (fallback):",
      vendorOrgId
    );
    return vendorOrgId;
  } catch (error) {
...
const verifyJwtToken = async (req, res, next) => {
  try {
    ...
    // Master keys should be stored in api_keys table linked to admin@example.com
    ...
    if (supabase) {
      ...
      console.log(
        "[maas-api] Validating API key, hash:",
      ...
      if (hashMatch) {
        ...
        if (keyMatch) {
          ...
          console.log(
            "[maas-api] Found key in legacy key column (migration period)"
          );
      ...
      if (!apiKeyRecord) {
        ...
        console.log(
          "[maas-api] API key not found in database, trying other auth methods..."
        );
      } else {
        // Key found - validate and set user context
        console.log(
          "[maas-api] API key validated successfully for user:",
        ...
        if (supabase && apiKeyRecord.user_id) {
          try {
            ...
            console.warn(
              "[maas-api] Exception fetching user organization_id:",
              userError.message
            );
          }
        }
    ...
    if (token.includes(".") && token.startsWith("pk_")) {
      ...
      console.log(
        "[maas-api] Resolved organization_id for vendor key (pk_ format):",
        organizationId
      );
      ...
      try {
        ...
        try {
          const introspectResponse = await fetch(
            `${authGatewayUrl}/oauth/introspect`,
            {
              method: "POST",
              headers: {
          ...
          req.user = {
            ...
            scope: Array.isArray(introspection.scope)
              ? introspection.scope
...
app.post("/api/v1/memory", async (req, res) => {
  try {
    ...
    if (!title || !content) {
      console.error(
        "[maas-api] Validation failed - title:",
        title,
        "content:",
        content
      );
    ...
    if (bodyOrgId) {
      ...
      if (req.user) {
        ...
        console.log(
          "[maas-api] Using organization_id from request body:",
          organizationId
        );
      }
    }
    ...
    if (!organizationId && req.user?.vendor_org_id && supabase) {
      console.log(
        "[maas-api] Attempting to resolve organization_id from vendor_org_id:",
        req.user.vendor_org_id
      );
    ...
    if (!organizationId && supabase && req.user?.id) {
      try {
        ...
        if (userError) {
          console.warn(
            "[maas-api] Error fetching user organization_id from users table:",
            userError.message
          );
        } else if (userData?.organization_id) {
          organizationId = userData.organization_id;
          console.log(
            "[maas-api] Found organization_id from users table in endpoint:",
            organizationId
          );
        }
      } catch (userError) {
    ...
    if (!organizationId) {
      ...
      return res.status(400).json({
        error:
          "Organization ID is required. User must be associated with an organization.",
        code: "MISSING_ORG_ID",
        debug: {
          sources_checked: [
            "request_body",
...
app.put("/api/v1/memory/:id", async (req, res) => {
  try {
    ...
    let organizationId =
      bodyOrgId || req.user?.organization_id || req.user?.vendor_org_id;
