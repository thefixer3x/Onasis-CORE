...
class SimpleMCPServer {
  ...
  async start() {
    try {
      ...
      console.log(`🚀 Enhanced MCP WebSocket Server running on ws://localhost:${this.config.wsPort}`);
      console.log(`📊 Health check available at http://localhost:${this.config.port}/health`);
      console.log(`🔒 Production hardening: maxPayload=1MB, API key validation, heartbeat enabled`);
      ...
      logger.info(`WebSocket endpoint: ws://${this.config.host}:${this.config.wsPort}/`);
        ...
        return {
          contents: [{
            ...
            text: JSON.stringify({
              ...
              connections: this.connections.size,
              uptime: process.uptime(),
              features: ['WebSocket MCP', 'Enterprise Orchestration', 'Memory Management']
            }, null, 2)
          }]
        };
