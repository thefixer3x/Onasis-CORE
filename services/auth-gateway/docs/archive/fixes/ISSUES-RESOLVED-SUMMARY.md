...
```typescript
const jobData = job.data as { memoryId: string; content: string; userId: string };
```
...
### **Master API Key:**
- **Raw:** `lano_master_key_2024` (for server-to-server)
- **Hashed:** `SHA256("lano_master_key_2024")` (for IDE extensions)
- **Both now supported** ✅
