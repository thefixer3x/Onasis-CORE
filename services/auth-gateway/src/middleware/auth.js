...
export function requireScopes(...requiredScopes) {
    return (req, res, next) => {
        ...
        const hasScope = requiredScopes.some(required => userScopes.some(userScope => scopeMatches(userScope, required)));
        if (!hasScope) {
            return res.status(403).json({
                error: 'Insufficient scope',
                code: 'SCOPE_INSUFFICIENT',
...
export function requireAllScopes(...requiredScopes) {
    return (req, res, next) => {
        ...
        const missingScopes = requiredScopes.filter(required => !userScopes.some(userScope => scopeMatches(userScope, required)));
