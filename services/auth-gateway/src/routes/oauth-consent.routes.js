...
const SUPABASE_URL = env.SUPABASE_URL || 'https://ptnrwrgzrsbocgxlpvhd.supabase.co';
...
router.get('/consent', (req, res) => {
    ...
    res.setHeader('Content-Security-Policy', `default-src 'self'; script-src 'self' 'nonce-${nonce}' https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline'; connect-src 'self' ${SUPABASE_URL} https://*.supabase.co; img-src 'self' data: https:;`);
    res.send(renderConsentPage(authorization_id, nonce));
});
/**
 * Renders the terminal-style consent page
 */
function renderConsentPage(authorizationId, nonce) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
