...
import { ORGANIZATIONAL_SUBDOMAINS } from '../../config/domain-config.js';
import { logger } from '../utils/logger.js';
/**
 * Auto-register OAuth client for a new organizational subdomain
 */
export async function autoRegisterSubdomainClient(subdomainConfig) {
    try {
...
function generateRedirectUris(config) {
    ...
    switch (config.type) {
        case 'app':
            standardCallbacks.push(`${baseUrl}/auth/success`, `${baseUrl}/login/callback`);
            break;
        case 'api':
            standardCallbacks.push(`${baseUrl}/v1/auth/callback`, `${baseUrl}/auth/token/callback`);
            break;
        case 'admin':
            standardCallbacks.push(`${baseUrl}/admin/auth/callback`, `${baseUrl}/dashboard/auth/callback`);
            break;
        case 'platform':
            standardCallbacks.push(`${baseUrl}/platform/auth/callback`, `${baseUrl}/auth/platform/callback`);
            break;
    }
 ...
 */
function generateScopes(config) {
 ...
 */
export async function registerAllOrganizationalSubdomains() {
 ...
 */
export function isAuthorizedRedirectUri(uri) {
    ...
    return organizationalDomains.some(domain => uri.includes(domain) ||
