...
export class ProjectServiceError extends Error {
    statusCode;
    code;
    constructor(message, statusCode = 500, code = 'PROJECT_SERVICE_ERROR') {
        super(message);
...
export async function createProject(ownerId, input) {
    ...
    try {
      ...
      `, [name, description, input.organizationId, ownerId, teamMembers, JSON.stringify(settings)]);
        ...
        if (error.code === '23505') {
            throw new ProjectServiceError('A project with this name already exists in the organization', 409, 'PROJECT_CONFLICT');
        }
...
export async function updateProject(projectId, userId, role, updates) {
...
export async function deleteProject(projectId, userId, role) {
    const project = await fetchProjectForUser(projectId, userId, role);
    ...
    const client = await getClientWithSchema();
    try {
        const result = await client.query(`DELETE FROM api_key_projects WHERE id = $1`, [projectId]);
...
export async function listProjectKeys(projectId, userId, role) {
...
async function fetchProjectForUser(projectId, userId, role) {
    if (!isUuid(projectId)) {
...
function normalizeTeamMembers(teamMembers, ownerId) {
...
function wrapUnknownError(error, message) {
    ...
    const err = error;
    return new ProjectServiceError(`${message}: ${err.message || 'Unknown error'}`, 500, 'PROJECT_SERVICE_ERROR');
}
