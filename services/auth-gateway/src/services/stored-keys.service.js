...
export class StoredKeyServiceError extends Error {
    statusCode;
    code;
    constructor(message, statusCode = 500, code = 'STORED_KEY_ERROR') {
        super(message);
 ...
 */
export async function listStoredKeysForProject(projectId, userId, role) {
 ...
 */
export async function getStoredKeyById(projectId, keyId, userId, role) {
    // Verify user has access to project
 ...
 * Validates user has access to the project
 */
export async function createStoredKey(projectId, userId, role, input) {
    // Verify user has access to project
    const project = await getProjectById(projectId, userId, role);
    ...
    try {
      ...
      `, [
        ...
        if (error.code === '23505') {
            throw new StoredKeyServiceError('A key with this name already exists in the project', 409, 'KEY_CONFLICT');
        }
 ...
 * Validates user has access to the project
 */
export async function updateStoredKey(projectId, keyId, userId, role, updates) {
 ...
 * Validates user has access to the project
 */
export async function deleteStoredKey(projectId, keyId, userId, role) {
    // Verify key exists and user has access
    await getStoredKeyById(projectId, keyId, userId, role);
    const client = await getClientWithSchema();
    try {
        const result = await client.query(`DELETE FROM stored_api_keys WHERE id = $1 AND project_id = $2`, [keyId, projectId]);
...
function mapStoredKeyRecord(record) {
    return {
        id: record.id,
        name: record.name,
...
function wrapUnknownError(error, message) {
    if (error instanceof StoredKeyServiceError) {
        return error;
    }
    ...
    const err = error;
    return new StoredKeyServiceError(`${message}: ${err.message || 'Unknown error'}`, 500, 'STORED_KEY_ERROR');
}
