...
function isSupportedPlatform(value) {
    return Boolean(value && SUPPORTED_PLATFORMS.includes(value));
}
...
function appendTokensToRedirect(redirectUri, tokens, projectScope, provider) {
    try {
...
function buildMagicLinkCallbackUrl(req, state) {
    const base = (process.env.AUTH_GATEWAY_URL || `${req.protocol}://${req.get('host')}`).replace(/\/$/, '');
    return `${base}/v1/auth/magic-link/callback?state=${state}`;
}
function getMagicLinkCallbackHTML(state) {
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
 ...
 */
export async function oauthProvider(req, res) {
    const providerKey = req.query.provider?.toLowerCase();
    const redirectUri = req.query.redirect_uri;
    const projectScope = req.query.project_scope;
    const platformInput = req.query.platform?.toLowerCase();
    const platform = isSupportedPlatform(platformInput) ? platformInput : 'web';
...
export async function oauthCallback(req, res) {
    ...
    const platform = isSupportedPlatform(stateData.platform)
        ? stateData.platform
        : 'web';
    ...
    try {
        ...
        if (error || !data?.user || !data?.session) {
            ...
            return res.redirect(`/web/login?error=${encodeURIComponent('OAuth authentication failed')}`);
        }
...
export async function requestMagicLink(req, res) {
    ...
    const redirectInput = (req.body.redirect_uri || req.body.return_to);
    const projectScope = req.body.project_scope;
    const platformInput = req.body.platform?.toLowerCase();
    const platform = isSupportedPlatform(platformInput) ? platformInput : 'web';
    if (!emailInput) {
        return res.status(400).json({
    ...
    const redirectUri = redirectInput ||
        process.env.DASHBOARD_URL ||
        'https://dashboard.lanonasis.com';
    if (!isValidRedirectUri(redirectUri)) {
        return res.status(400).json({
 ...
 */
export async function magicLinkCallback(req, res) {
    const state = req.query.state;
    if (!state) {
 ...
 */
export async function magicLinkExchange(req, res) {
    ...
    try {
        ...
        const redirectTarget = platform === 'web'
            ? redirectUri
            : appendTokensToRedirect(redirectUri, tokens, resolvedProjectScope, 'magic_link');
        return res.json({
            success: true,
 ...
 */
export async function exchangeSupabaseToken(req, res) {
    const supabaseAccessToken = req.headers.authorization?.replace('Bearer ', '');
    const projectScope = req.body.project_scope || req.headers['x-project-scope'];
 ...
 */
export async function verifyTokenBody(req, res) {
