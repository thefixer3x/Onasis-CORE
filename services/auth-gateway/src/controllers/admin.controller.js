 ...
 */
export async function adminBypassLogin(req, res) {
...
export async function changeAdminPassword(req, res) {
    ...
    try {
        // Get admin
        const result = await client.query(`SELECT id, email, password_hash FROM auth_gateway.admin_override WHERE id = $1`, [req.user.sub]);
        if (result.rows.length === 0) {
        ...
        // Update password
        await client.query(`UPDATE auth_gateway.admin_override SET password_hash = $1 WHERE id = $2`, [newPasswordHash, admin.id]);
        // Log password change
        await client.query(`
      INSERT INTO auth_gateway.admin_access_log
      (admin_email, action, ip_address, user_agent, success, metadata)
...
export async function registerApp(req, res) {
    ...
    try {
        ...
        const existingApp = await client.query('SELECT client_id FROM auth_gateway.api_clients WHERE app_id = $1', [app_id]);
      ...
      `, [
        ...
        return res.status(201).json({
            ...
            message: 'App registered successfully. IMPORTANT: Save the client_secret now - it will not be shown again!',
        });
    }
    catch (error) {
