import { StoredKeyServiceError, createStoredKey, deleteStoredKey, getStoredKeyById, listStoredKeysForProject, updateStoredKey, } from '../services/stored-keys.service.js';
/**
 * List all stored API keys for a project
 * GET /api/v1/projects/:projectId/api-keys
 */
export async function listProjectStoredKeys(req, res) {
    if (!req.user?.sub) {
 ...
 */
export async function getStoredKey(req, res) {
    if (!req.user?.sub) {
        return res.status(401).json({
            error: 'Authentication required',
            code: 'AUTH_REQUIRED',
    ...
    try {
        const key = await getStoredKeyById(req.params.projectId, req.params.keyId, req.user.sub, req.user.role);
        return res.json(key);
    }
    catch (error) {
        return handleStoredKeyError(res, error);
 ...
 */
export async function createStoredKeyHandler(req, res) {
    if (!req.user?.sub) {
        return res.status(401).json({
            error: 'Authentication required',
            code: 'AUTH_REQUIRED',
 ...
 */
export async function updateStoredKeyHandler(req, res) {
    if (!req.user?.sub) {
        return res.status(401).json({
            error: 'Authentication required',
            code: 'AUTH_REQUIRED',
    ...
    try {
        const key = await updateStoredKey(req.params.projectId, req.params.keyId, req.user.sub, req.user.role, {
            name: req.body?.name,
            encryptedValue: req.body?.encryptedValue,
            keyType: req.body?.keyType,
            environment: req.body?.environment,
 ...
 */
export async function deleteStoredKeyHandler(req, res) {
    if (!req.user?.sub) {
        return res.status(401).json({
            error: 'Authentication required',
            code: 'AUTH_REQUIRED',
...
function handleStoredKeyError(res, error) {
