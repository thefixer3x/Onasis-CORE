...
class EnhancedMCPWebSocketHandler {
  ...
  verifyClient(info) {
    try {
      ...
      if (!apiKey) {
        this.logger.warn(
          "WebSocket connection attempt without API key - allowing for testing",
        );
      ...
      if (!isValidFormat) {
        this.logger.warn(
          "WebSocket connection attempt with invalid API key format",
        );
        return false;
      }
  ...
  async handleMessage(ws, data) {
    try {
      ...
      this.logger.info(
        `MCP message received from ${connection.sessionId}: ${message.method || message.type}`,
      );
  ...
  // Health and Status Tools
  async getHealthStatus(_args, _connection) {
    return {
      status: "healthy",
        ...
        contents: [
          {
            ...
            mimeType: "application/json",
            text: JSON.stringify(
              { message: "Resource content would be here" },
              null,
              2,
            ),
          },
  ...
  async start() {
    ...
    this.logger.info(
      `Standalone WebSocket MCP server listening on port ${this.port}`,
    );
    return Promise.resolve();
  }
