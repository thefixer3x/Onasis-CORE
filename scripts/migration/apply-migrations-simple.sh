...
if [ -z "$DATABASE_URL=postgresql://<user>:<password>@<host>:<port>/<db>
    if [ -f "services/auth-gateway/.env" ]; then
        export $(grep -v '^#' services/auth-gateway/.env | xargs)
    else
        echo "ERROR: DATABASE_URL=postgresql://<user>:<password>@<host>:<port>/<db>
        exit 1
    fi
fi

DB_URL="$DATABASE_URL=postgresql://<user>:<password>@<host>:<port>/<db>
